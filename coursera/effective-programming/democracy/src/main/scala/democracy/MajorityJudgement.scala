...
/** A grade to assign to a candidate. There are seven possible grades (from the worst to the best): `Bad`, `Mediocre`,
  * `Inadequate`, `Passable`, `Good`, `VeryGood`, and `Excellent`.
  *
...
object Grade:
  /** @return
    ...
    *
    * The median grade can be computed by sorting the collection and taking the element in the middle. If there are an
    * even number of grades, any of the two grades that are just before of after the middle of the sequence are correct
    * median values.
    *
    * Grades can be compared by using their `ordinal` method.
    *
