...
case class Waiting(countDown: Int, startPosition: Point, numberOfParticles: Int, particlesColor: Color)
  ...
  /** @return
    ...
    *
    * If the [[countDown]] is greater than zero, the firework stays in the [[Waiting]] state and simply decrements the
    * [[countDown]] of one unit. Otherwise, it transitions to the [[Launched]] state.
    *
...
case class Launched(countDown: Int, position: Point, direction: Angle, numberOfParticles: Int, particlesColor: Color)
  ...
  /** @return
    *   The next state of this firework
    *
    * As long as the [[countDown]] is greater than zero, the firework stays in the [[Launched]] state: it moves one step
    * further in its [[direction]] and decrements its [[countDown]] of one unit. Otherwise, it transitions to the
    * [[Exploding]] state.
    *
    * Hints: use the operation [[Motion.movePoint]] to compute the next position of the firework, use the operation
    * [[Exploding.init]] to transition the firework to the [[Exploding]] state, and use the constant
...
case class Exploding(countDown: Int, particles: Particles) extends Firework:
  ...
  /** @return
    *   The next state of this firework
    *
    * As long as the [[countDown]] is greater than zero, the firework stays in the [[Exploding]] state: it updates the
    * state of its [[particles]] and decrements its [[countDown]]. Otherwise, it transitions to the [[Done]] state.
    *
    * Hint: use the operation [[Particles.next]] to compute the next state of the particles of this firework.
    */
  def next: Firework =
...
case class Particle(horizontalSpeed: Double, verticalSpeed: Double, position: Point, color: Color):
  ...
  /** @return
    ...
    *
    * Hint: use the operation [[Motion.drag]] to simulate the friction with the surrounding air, and the constant
    * [[Settings.gravity]] to simulate the gravity.
    */
  def next: Particle =
...
object Motion:
  ...
  /** @return
    *   The next position of the given `point`, assuming that it moves towards the given `direction` at the given
    *   `speed`
    * @param point
